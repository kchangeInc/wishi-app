# shared/repository/pg/cluster.py
"""PostgreSQL cluster repository - handles cluster CRUD operations"""
...
class PgClusterRepository(PgBaseRepository[Cluster], IClusterRepository):
    """PostgreSQL cluster-specific repository"""
    ...
    def update_cluster(self, id: int, user_id: int = None, **kwargs) -> Optional[Cluster]:
    ...
    def soft_delete_cluster(self, id: int, user_id: int = None) -> Optional[Cluster]:
        """Soft delete cluster"""
        cluster = self.get_not_deleted(id)
        if cluster:
