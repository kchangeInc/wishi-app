# shared/repository/pg/match.py
"""PostgreSQL match repository - handles match CRUD operations"""

import logging
from sqlalchemy.orm import Session
from shared.models import Match
from shared.repository.pg.base import PgBaseRepository
...
class PgMatchRepository(PgBaseRepository[Match], IMatchRepository):
    """PostgreSQL match-specific repository"""
    ...
    def soft_delete_match(self, id: int, user_id: int = None) -> Optional[Match]:
        """Soft delete match"""
        match = self.get_by_id(id)
        if match:
    ...
    def get_by_status(self, status: str, skip: int = 0, limit: int = 100) -> List[Match]:
        """Get matches filtered by a single status"""
        return self.db.query(Match).filter(
            Match.status == status,
            Match.is_deleted == False
        ).order_by(Match.score.desc()).offset(skip).limit(limit).all()
