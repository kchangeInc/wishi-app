...
def get_notifications(
    ...
    skip: int = 0, limit: int = 50, repos: RepositoryFactory = Depends(get_repos)
):
    user = get_current_user(request)
    repo = repos.notification()
...
def mark_read(notification_id: int, request: Request, repos: RepositoryFactory = Depends(get_repos)):
    user = get_current_user(request)
    repo = repos.notification()
...
def mark_all_read(request: Request, repos: RepositoryFactory = Depends(get_repos)):
    user = get_current_user(request)
    repo = repos.notification()
...
def delete_notification(notification_id: int, request: Request, repos: RepositoryFactory = Depends(get_repos)):
    user = get_current_user(request)
    repo = repos.notification()
...
def clear_all_notifications(request: Request, repos: RepositoryFactory = Depends(get_repos)):
    user = get_current_user(request)
    repo = repos.notification()
...
def get_notification_settings(request: Request, repos: RepositoryFactory = Depends(get_repos)):
    user = get_current_user(request)
    repo = repos.notification_setting()
...
def update_notification_settings(body: NotificationSettingUpdate, request: Request, repos: RepositoryFactory = Depends(get_repos)):
    user = get_current_user(request)
    repo = repos.notification_setting()
...
def notify(req: NotifyRequest):
    ...
    return {"status": "ok", "delivered_to": len(req.recipients), "timestamp": datetime.now(timezone.utc).isoformat()}
